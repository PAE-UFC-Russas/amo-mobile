...
export default function ModalDetailScheduling({setOpenModal, openModal, details, subjects}){
    const { user } = useAuth();
    const HandleOnClose = () =>{
        setOpenModal(false)
    }
    ...
    const EnableConfirmSchedule = () => {
        const isMonitor = subject.monitores.find(obj => obj.id == user.perfil.id)?true:false;
        const isProfessor = subject.professores.find(obj => obj.id == user.perfil.id)?true:false;
            ...
            >
                ...
                <View width='85%' height='54%' alignSelf='center' borderWidth={1} borderColor='grey' borderRadius={5} marginTop='5%' padding='3%'>
                    ...
                    <View marginBottom='2%'>
                        ...
                        <Text fontSize={15}>{subject.monitores.map((item)=>{return item.nome_exibicao})}</Text>
                    </View>
                    <View marginBottom='2%'>
